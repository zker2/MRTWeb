...
// Responsive map height handler
function resizeMap() {
  const mapContainer = document.getElementById("map");
  if (!mapContainer) return;
  const height = window.innerWidth < 768 ? Math.max(window.innerHeight * 0.6, 500) : 700;
...
document.addEventListener("DOMContentLoaded", resizeMap);  // เพิ่มเพื่อรองรับ DOM โหลดเสร็จ
...
map.whenReady(() => {
  addGridLines();
  resizeMap();  // ✅ บังคับให้ขนาดถูกต้องหลัง map พร้อม
});
...
if (navigator.geolocation) {
  navigator.geolocation.getCurrentPosition(
    (position) => {
      ...
      resizeMap();  // ✅ รีเฟรชขนาดหลัง setView
    },
    () => {
...
function calculateRoute() {
  ...
  resizeMap(); // ✅ บังคับแสดงผล map ก่อน routing
